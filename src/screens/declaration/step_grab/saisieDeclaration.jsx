...
import getReceivedRecuActions from '../../../redux/declaration/getReceivedRecu'
...
const SaisieDeclaration = ({
    ...
    const saisieAction = ({ id, index }) => {
        history.push({
            pathname: `/declaration_saisie/saisie_details_declaration/${id}`,
            state: {
                idDeclaration: id,
                lang: lng,
    ...
    const getReceviedAction = ({ id }) => {
        getReceivedRecuReq({ idDeclaration: id, payload: { type: 'reçu' } })
    }
    ...
    return (
        <div style={{ padding: '1%' }}>
            <Grid className="gridItem">
                <PageTitle label="إدخال البيانات" />
                <label
                    htmlFor="form"
...
const mapDispatchToProps = dispatch => ({
    ...
    getReceivedRecuReq: payload =>
        dispatch(getReceivedRecuActions.getReceivedRecuRequest(payload)),
})
/**
 *  Inialisation
 */
SaisieDeclaration.defaultProps = {
    filtredTable: [],
}
/**
 *  declaration des props
 */
SaisieDeclaration.propTypes = {
    ...
    getReceivedRecuReq: PropTypes.func.isRequired,
}

export default connect(
    mapStateToProps,
    mapDispatchToProps
)(injectIntl(SaisieDeclaration))
